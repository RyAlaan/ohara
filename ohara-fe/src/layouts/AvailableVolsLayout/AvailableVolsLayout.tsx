...
const AvailableVolsLayout = ({ books }: AvailableVolsLayoutInterface) => {
  return (
    <div className="w-full md:w-7/12 min-h-96 max-w-full p-6 bg-white rounded-md">
      <div className="w-full max-w-full overflow-auto *:w-full *:px-1.5 *:md:px-4 *:py-2 *:flex *:flex-row *:items-start *:border-b *:border-dashed *:gap-x-4 *:justify-between">
        ...
        {books &&
          books.slice(0, 8).map((book: BookInterface) => (
            <div className="h-full *:h-full *:flex *:items-center *:text-sm">
